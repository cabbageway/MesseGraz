...
class Database
{
    ...
    */
    ...
    {
        $sql = "SELECT DISTINCT gutschein_id, titel, image, kategorie.bezeichnung katbez, gutschein.kategorie_id cat, beschreibung, limitierung, stand_number FROM gutschein 
    ...
    public function insertCoupon($file, $title, $category, $content, $fair, $number, $limitierung)
    {
        $nextId = 0;
        if (empty($limitierung) || str_contains($limitierung, '/') || $limitierung == 0) {
            $sql = "INSERT INTO gutschein (image, kategorie_id, titel, beschreibung, messe_id, stand_number) VALUES ('" . strtolower($file) . "', " . $category . ", '" . $title . "','" . $content . "','" . $fair . "','" . $number . "');";
        } else {
            $sql = "INSERT INTO gutschein (image, kategorie_id, titel, beschreibung, messe_id, stand_number, limitierung) VALUES ('" . strtolower($file) . "', " . $category . ", '" . $title . "','" . $content . "','" . $fair . "','" . $number . "','" . $limitierung . "');";
        }
    ...
    public function updateCoupon($id, $file, $title, $category, $content, $fair, $number, $limitierung)
    {
        if (empty($limitierung) || str_contains($limitierung, '/') || $limitierung == 0) {
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)) {
                ...
                mysqli_stmt_bind_param($stmt, "sissisi", $file, $category, $title, $content, $fair, $number, $id);
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)) {
                ...
                mysqli_stmt_bind_param($stmt, "sissisii", $file, $category, $title, $content, $fair, $number, $limitierung, $id);
                mysqli_stmt_execute($stmt);
            }
        }
